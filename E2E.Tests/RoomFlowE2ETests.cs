...
public class RoomFlowE2ETests(AppHostFixture fixture)
{
	...
	// 原本這一條叫 Update_RunsTheScriptAgainstRealRedis，為了驗 B1 那段 EXISTS 守門的 Lua 真的跑
	// 得起來。**B2 把房間搬到 Postgres，那段 Lua 連同 RedisRoomStore 一起消失了**——`UPDATE ...
	// WHERE room_id = $1` 影響 0 列本來就回 false，不需要守門。
	//
	// 這一條留著，理由換成「改設定這條命令走完整條真實鏈路（含 Postgres）之後，讀回來是對的」
	// ——它是 room.update 在 E2E 唯一的覆蓋。守門那個性質現在由契約測試
	// TryUpdateSettings_DoesNotResurrectADeletedRoom 在真 Postgres 上驗（PostgresContractTests）。
	[E2EFact]
	public async Task Update_ClearsThePassword_ThroughTheRealChain()
	{
