...
狀態：核心設計與實作已完成（Gateway / Dispatcher / `Common.Connections` / `Common.Delivery`，含單元測試）；ADR-9 的 handshake 驗證與 principal 轉交**尚未實作**，見第 9 節
...
- **待實作（ADR-9）**：`Origin` allowlist、`IConnectionAuthenticator` 的兩個呼叫點、`Connection.Principal`、`ConnectionLifecycle.OnConnectedAsync(socket, principal, ...)`、`IInboundMessageHandler` 加 `principal` 參數、`ConnectionDisconnected` 加 `principal` 欄位、Gateway 的 `AddKeyedRedisClient("identity-store")`。這批改動會同時動到協定層（`InboundPacket`、`CommandContext`）與身分層（`Common.Identity`），是一次跨三層的介面同步變更，建議一個 commit 完成以免中間狀態編不過。
