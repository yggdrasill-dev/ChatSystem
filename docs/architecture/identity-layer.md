...
- ADR-8 反向索引 `ConnectionUser:{connectionId}` 的 TTL 長度與續期方式尚未決定，但**選項變好了**：連線層已經新增 `events.connection.disconnected`（`connection-layer.md` 第 6.7 節、ADR-8），本層可以訂閱它、在斷線時直接刪掉正反兩個 key，不必再用「filter 每次讀到就續期」那種會誤殺閒置連線的做法。仍然要留一個長 TTL 當保險，因為連線層明確把事件定為 best-effort（process 被 kill 時發不出來）——但那個 TTL 只是防洩漏的兜底，不再是正確性的依據，所以可以設得很寬鬆。
